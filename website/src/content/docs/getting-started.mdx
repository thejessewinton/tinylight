...
> **Please note that tinylight is currently in a very early beta stage.** It is not recommended for use in production quite yet, but will be ready within the next few weeks.

# Getting Started

The core principle behind tinylight has take a lot of inspiration from [HeadlessUI](https://headlessui.com) or [Radix](https://radix-ui.com). I'm a big fan of both libraries and I wanted to create something similar for components that are less often considered. Right now, that means lightboxes, but the idea is to expand this to some other components as well. Right now the roadmap includes an oEmbed container for Vimeo and YouTube players, and HTML video and audio components.
